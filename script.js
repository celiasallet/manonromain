...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (form) {
    form.addEventListener('submit', e => {
      ...
      btn.disabled = true;
      form.submit(); // envoi vers Google Forms
    });
  }
  ...
  function renderTrips(trips) {
    ...
    trips.forEach(trip => {
      ...
      card.innerHTML = `
        ...
        <p class="dispo">Places : <strong><span class="seats-left">${trip.seats_left}</span> / ${trip.seats_total}</strong></p>
        ...
        button.addEventListener('click', () => {
          ...
          .then(data => {
            if(data.success){
              ...
              // Ajouter la réservation dans le tableau séparé
              const myResContainer = document.getElementById('my-reservations');
              const resItem = document.createElement('div');
              resItem.className = 'reservation-item';
              resItem.textContent = `${pseudo} - Départ : ${trip.departure}`;
              myResContainer.appendChild(resItem);
