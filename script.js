document.addEventListener('DOMContentLoaded', () => {
  ...
  if (tripForm) {
    tripForm.addEventListener('submit', e => {
      ...
      fetch(API_URL, { method: 'POST', body: JSON.stringify(tripData) })
        .then(res => res.json())
        .then(data => {
          if (data.success) {
  ...
  function renderTrips(trips) {
    ...
    trips.forEach(trip => {
  // 🚫 NE PAS AFFICHER si seats_total ET seats_left valent 1
  ...
  if (trip.seats_left >= 1) {
    ...
    button.addEventListener('click', () => {
      ...
      fetch(API_URL, { method: 'POST', body: JSON.stringify({ action: 'reserve', trip_id: trip.id, pseudo }) })
        .then(res => res.json())
        .then(data => {
          if (data.success) {
  ...
  if (tripsContainer) {
    fetch(API_URL)
      ...
      .then(data => {
        ...
        const mainTrips = tripsData
          .filter(t => t.seats_total >= 1)
          .filter(t => !(t.seats_total === 1 && t.seats_left === 1)); // <-- règle 1/1
...
//         const pseudo = input.value.trim();
//         if(!pseudo){
//           showPopupInCard(card, "Merci de mettre ton pseudo pour réserver une place");
//           return;
//         }
